...
export default function Homepage() {
  ...
  useEffect(() => {
    const url = `https://cors-anywhere.herokuapp.com/https://api.musixmatch.com/ws/1.1
		/chart.tracks.get?chart_name=top&page=1&page_size=10&country=us&apikey=${process.env.REACT_APP_KEY}`;
  ...
  const trackCard = trackList.map((item) => <TrackCard track={item.track} key={item.track.track_id} />);
