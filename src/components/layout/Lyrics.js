...
export default function Lyrics(props) {
  ...
  useEffect(() => {
    ...
    Promise.all([fetch(url + getName + key), fetch(url + getLyrics + key)])
      .then(([nameRes, lyricsRes]) => Promise.all([nameRes.json(), lyricsRes.json()]))
      .then(([nameData, lyricsData]) => {
        setTrackName(JSON.stringify(nameData.message.body.track.track_name).replace(/"/g, ""));
        setArtistName(JSON.stringify(nameData.message.body.track.artist_name).replace(/"/g, ""));
